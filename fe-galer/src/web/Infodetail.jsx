...
export default function Infodetail() {
  ...
  useEffect(() => {
    const fetchDetailInformasi = async () => {
      try {
        const response = await axios.get(`/api/webnews/${id}`);
  ...
  if (!detail) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-xl text-gray-500">Informasi tidak ditemukan.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-16">
      <div className="max-w-4xl mx-auto py-12 px-6 bg-white shadow-lg rounded-lg">
        <a
          data-fancybox="info-image"
          href={constructImageUrl(detail.gambar)}
          data-caption={detail.judul}
